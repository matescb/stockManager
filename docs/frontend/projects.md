...
Sourcing filters are constrained to the workspace active lists from `GET /api/workspaces/current`: country and currency render native dropdowns from `active_countries` and `active_currencies`, and distributors render the same checkbox multi-select pattern used by workspace settings. The saved workspace defaults are selected when active; if settings drift leaves a saved default outside the active list, the page falls back to the first active option and shows a warning. Source: `web/src/routes/projects/sourcing/ProjectSourcingPage.tsx`.
