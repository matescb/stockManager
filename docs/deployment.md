...
|---|---|---|
| `category-seed` | Creates the missing categories of the 28-row seed tree per workspace — passives with their sub-classes, plus ICs, Connectors, Crystals & Oscillators, Fuses, Switches, Transformers and Mechanical — and fills KiCad metadata that was never set. Never renames, re-parents or overwrites. | CSV: `workspace_id, workspace_name, path, action, category_id, detail` |
...
| `provider-refresh` | Re-runs the provider MPN lookup for every active, linked part with an MPN and writes back what the providers answer now, through the same service the refresh route uses. Drives the part columns on the primary tier, fills a NULL category, normalises the specs, fetches missing assets. Requires an exact MPN match; `--link-missing-providers` adds SECONDARIES only and never promotes a primary. Reads flags of its own: `--limit N`, `--only-uncategorized`, `--link-missing-providers`, `--sleep-ms MS`. | CSV: `workspace_id, part_id, mpn, provider, tier, action, part_columns_changed, specs_added, specs_updated, specs_restored, specs_removed, category_before, category_after, assets_fetched, assets_would_fetch, error`, plus counts per provider per workspace and the raw keys the schema had no alias for |
