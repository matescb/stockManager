...
- Don't sort or filter on `value_num` without accounting for the NULLs; `ix_custom_fields_ws_key_value_num` is partial (`WHERE value_num IS NOT NULL`), so a filter needs that predicate and an ORDER BY needs `NULLS LAST` plus a text tiebreaker — the parts list's spec sort orders `value_num NULLS LAST, value NULLS LAST, id` for exactly this reason (`domain/parts/services/spec_columns.py::sorted_page`).
