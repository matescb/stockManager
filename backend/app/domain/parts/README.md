...
3. **Catalog vs spec keys are split.** `web/src/lib/providerCatalog.ts` and `services/provider.py` (or equivalent) must agree on which custom-field keys are catalog metadata vs user specs. See [ADR-0007](../../../../docs/adr/0007-provider-catalog-vs-spec-split.md).
...
- Don't add a new "catalog" custom-field key on the server without updating `web/src/lib/providerCatalog.ts`. The Specs vs Sourcing tabs split on this list.
