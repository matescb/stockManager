...
| Whose ROW is this? (refresh) | `provider_fields.py::provider_owns_custom_field_row` |
| Whose ROW is this? (unlink) | `provider_fields.py::provider_wrote_custom_field_row` |
...
- Don't widen a provider reconciliation past `provider_owns_custom_field_row`. Its trailing pass deletes every `source='provider'` row absent from the payload; unscoped, that eats every other provider's rows on each refresh. `tests/test_secondary_provider.py` pins both directions.
