...
| Resolve a category's Value rules | `kicad_specs.py::rules_by_category` |
| Render a Value from specs | `value_template.py::render_value` |
...
| Render a 2D preview (cached) | `render.py::get_or_build_svg` |
...
9b. **The package re-points a symbol's `Footprint` field at the footprint it ships.** A vendor library's symbols name the vendor's own footprint library (`NSW:USB_A_Molex`); once the footprint ships through the PCM it is registered as `PCM_SM_<slug>`, and `NSW` is registered nowhere. `pcm._symbol_for_package` rewrites the field when the entry it names is a hosted footprint and leaves any other reference (KiCad's stock libraries, the user's own) as stored. The rewrite is a function of the plan, so the cache key covers it; a change to the rewrite itself needs `pcm.PACKAGE_FORMAT` bumped, which is the version's major and what makes an installed copy notice a deploy.
9c. **A model linked on the CAD tab reaches KiCad only through the package.** `eda_footprint_models` is a join row; `link_footprint_model` touches the footprint's `updated_at` (so the package version moves) but never its bytes. `pcm._footprint_for_package` appends a `(model …)` node for every linked datafile the bytes don't already reference, in position order, and the cache fingerprint carries the linked names. The CAD tab's own 3D preview reads the link table directly, which is why a model can show there and be absent in KiCad if this is ever bypassed.
...
13b. **A symbol's `Value` is `part_eda.value`, then the category template, then `parts.name`.** Most of the library is named after the provider's description, so `parts.name` alone put marketing copy on the schematic. `part_categories.value_template` (alembic 0082) renders `10 kΩ 1% 0603` from the part's own `custom_fields`; `kicad_fields` emits chosen specs as hidden symbol fields, and the render is appended to `keywords` so the chooser can find the part by what it is. Both columns are NULL-means-inherit up `parent_id`, and an archived ancestor stops the walk. A spec key whose title-cased label collides with a built-in field is skipped — KiCad matches its mandatory fields case-insensitively, so a spec called `value` would be two Values. A placeholder is `{[a-z_]+}`, so it can only name a canonical key — a part still carrying a provider's verbatim attribute name (`Resistance`) renders nothing and keeps `parts.name`, which is the behaviour this replaces rather than a regression. The PCM package's library members are untouched by any of this: it ships library entries, not parts, so there is no per-part Value in it and `PACKAGE_FORMAT` did not move (the package version does, because `part_categories.updated_at` feeds it — rule 17).
...
- Don't render 2D previews in-process. `render.py` hands the geometry to the `kicad-render` sidecar (see `render/`) precisely so a ~200 MB KiCad install stays out of the slim backend image and no request path runs a subprocess. A sidecar that is down degrades to a 503 "preview unavailable"; it never 500s the app. Bump `render.py::KICAD_SERIES` and the sidecar's `Dockerfile` digest together — the series is baked into the SVG cache tag so a version change invalidates it.
- Don't confuse `render.py`'s symbol wrapper with `pcm.py`'s. Both put a bare `(symbol …)` into `(kicad_symbol_lib …)`, but `pcm.py` builds the package KiCad installs and `render.py` builds the throwaway one-symbol library it hands kicad-cli to draw. They share a shape, not a purpose.
