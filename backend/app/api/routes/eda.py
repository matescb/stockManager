...
from app.domain.eda import kicad_library, kicad_refs, pcm, render, storage
...
# 2D preview images
#
# The CAD tab draws hosted symbols and footprints exactly as KiCad does,
# by rendering them through kicad-cli in the `kicad-render` sidecar and
# serving the SVG it produces. `domain/eda/render.py` owns the sidecar
# call and the content-addressed SVG cache; these routes are the thin
# HTTP edge on top of it.
#
# Two things about these responses are load-bearing:
#
# * **`image/svg+xml` + `nosniff`, and the client renders it via `<img>`.**
#   The SVG is generated by kicad-cli from attacker-supplied stored
#   geometry and served from our own origin. An SVG opened as a document
#   can run script, so the frontend loads it in an `<img>` (which never
#   executes script) and `nosniff` stops a browser reinterpreting it as
#   anything else. See `web/src/components/eda/SvgPreview.tsx`.
...
# a cold render — a blocking call to the sidecar's kicad-cli — on the
# single uvicorn worker prod runs.
...
def _svg_response(path: str | None, data: bytes | None) -> Response:
    headers = {
        "Cache-Control": _PREVIEW_CACHE_CONTROL,
        # kicad-cli-rendered SVG derived from user geometry, served from
        # our own origin; never let a browser sniff it into a document. The
        # client renders it via <img>, which cannot execute any script the
        # SVG might carry.
        "X-Content-Type-Options": "nosniff",
        # Defence in depth for the case the <img> stance does not cover: a
        # browser that *navigates* straight to this URL renders the SVG as
        # a same-origin document, where embedded <script> would run. nginx
        # sets a CSP for /api in prod, but this route can be reached without
        # it, so lock it down at the app layer too (SEC2-009, same stance as
        # catalog.py). `sandbox` + `default-src 'none'` leaves nothing but
        # the drawing — no script, no navigation, no framing.
        "Content-Security-Policy": "default-src 'none'; style-src 'unsafe-inline'; sandbox",
        "X-Frame-Options": "DENY",
    }
    if path is not None:
        return FileResponse(path, media_type="image/svg+xml", headers=headers)
    return Response(content=data or b"", media_type="image/svg+xml", headers=headers)


def _build_svg(
    ws, row, *, kind: str, storage_kind: str
) -> tuple[str | None, bytes | None]:
    """Read the stored entry and render-or-serve-cached its SVG (sync).

    Runs whole on the threadpool: the blob read and the blocking sidecar
    call both belong off the event loop. A missing blob raises out of
    `_stored_bytes` (503) and propagates cleanly through `run_in_threadpool`.
    """
    raw = _stored_bytes(ws, row, kind=storage_kind)
    return render.get_or_build_svg(ws.id, row.sha256, raw, kind)


async def _svg_preview(ws, row, *, kind: str, storage_kind: str) -> Response:
    """Shared body of the two preview routes: render → SVG response.

    Every `render.RenderError` becomes a 503 "preview unavailable" (a
    too-large SVG a 413): the stored entry is our own validated content, so
    a render failure is a service condition, never the caller's fault.
    """
    try:
        path, data = await run_in_threadpool(
            _build_svg, ws, row, kind=kind, storage_kind=storage_kind
        )
    except render.OutputTooLarge:
        raise_http(
    ...
    return _svg_response(path, data)


@router.get("/symbols/{symbol_id}/preview.svg")
@limiter.limit(_PREVIEW_RATE, key_func=workspace_key)
async def get_symbol_preview(
    request: Request,
    symbol_id: UUID,
    db: DbSession,
    ws: CurrentWorkspace,
) -> Response:
    """The symbol drawn by kicad-cli, as SVG.
    ...
    return await _svg_preview(ws, row, kind="symbol", storage_kind=storage.SYMBOL_KIND)


@router.get("/footprints/{footprint_id}/preview.svg")
@limiter.limit(_PREVIEW_RATE, key_func=workspace_key)
async def get_footprint_preview(
    request: Request,
    footprint_id: UUID,
    db: DbSession,
    ws: CurrentWorkspace,
) -> Response:
    """The footprint drawn by kicad-cli, as SVG.
    ...
    return await _svg_preview(
        ws, row, kind="footprint", storage_kind=storage.FOOTPRINT_KIND
    )
